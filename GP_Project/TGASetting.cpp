...
__fastcall TGASetting::TGASetting(){;
	n_selections=5;
	n_crossovers=3;
	n_mutations=3;
}
//---------------------------------------------------------------------------
void TGASetting::Reset_probabilities(){
...
//---------------------------------------------------------------------------
void TGASetting::Recount_probabilities(const TGAPopulation &population){
	...
	int n_max=0;
		...
		Calculate_average_fitness(population);
...
//---------------------------------------------------------------------------
void TGASetting::Calculate_average_fitness(const TGAPopulation &population){
...
vector<int> amount_mutation(n_mutations,0);
	...
	for (int i=1; i < size_of_population; i++){ // !!Внимание клонирование не отключать, или изменить на i=0
		...
		if (amount_selection[type_of_selection[i]]!=0)
		 average_fitness_selection[type_of_selection[i]]+=population.genotype[i].fitness/amount_selection[type_of_selection[i]];

		if (amount_crossover[type_of_crossover[i]]!=0)
		 average_fitness_crossover[type_of_crossover[i]]+=population.genotype[i].fitness/amount_crossover[type_of_crossover[i]];

		if (amount_mutation[type_of_mutation[i]]!=0)
		 average_fitness_mutation[type_of_mutation[i]]+=population.genotype[i].fitness/amount_mutation[type_of_mutation[i]];
	}

}
void TGASetting::Select_operators(){
