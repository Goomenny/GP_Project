...
//---------------------------------------------------------------------------
/*TTree_symbolic& TTree_symbolic::operator=(const TTree_symbolic &other) {//перегрузка оператора присваивани€
...
//---------------------------------------------------------------------------
TTree_fuzzy& TTree_fuzzy::operator=(const TTree_fuzzy &other) {//перегрузка оператора присваивани€
...
vector< vector <int> > TTree_fuzzy::Decart(int *rule_index[], int num_term)
{
	...
	for (int i = 0; i < n_var; i++) {
		...
		for (int v = rule_index[0][i]; v < rule_index[1][i]; v++) {     //—брасываем начальные и конечные значени€
			tmp.push_back(v);
		}
		sets.push_back(tmp);
	}
	int const n = sets.size();
	vector<int> counter(n, 0); // счЄтчик размером с количество множеств
	...
	while (!carry)
	{
		...
		//¬ставл€ем номер выходного терма
		carthesian.push_back(num_term);
		//ƒобавл€ем правило в общий список новых правил
		res.push_back(carthesian);

		// выполн€ем инкремент счЄтчика - поразр€дно, начина€ с младшего (первого) разр€да
		carry = true;
		for (int i = 0; i != n && carry; ++i)
		{
			int& cell = counter[i];
			++cell;
			carry = cell == sets[i].size(); // у каждого разр€да - свой предел
			if (carry) cell = 0;
		}
		// если перенос вылетел за пределы счЄтчика - значит, он перекрутилс€
	}
	return res;
}
...
void TTree_fuzzy::Get_result(const vector<int>& n_terms) {
	...
	vector< vector<int> > new_rules; //ћассив добавл€емых правил в терминльном узле

	vector<int> operations;    //¬ектор с операци€ми %x%y
	int *rule_index[2];     //Ќачальные и конечные значени€ термов по переменным
	for (int i = 0; i < 2; i++) {
	...
	for (int i = 1; i < deep; i++) {
		for (int j = 0; j < n_node[i]; j++) {
			...
			if (!node[i][j].Get_type()) { //≈сли узел терминальный

				for (int v = 0; v < n_var; v++) {     //—брасываем начальные и конечные значени€
					rule_index[0][v] = 0;
					rule_index[1][v] = n_terms[v];
				}

				int num_parent = node[i][j].Get_num_parent();        //Ќомер родител€
				operations.clear();      //ќчищаем операции
				for (int i2 = i - 1; i2 >= 0; i2--) {    //ѕроходим цепочку вверх собира€ операции
					operations.push_back(node[i2][num_parent].Get_func());
					num_parent = node[i2][num_parent].Get_num_parent();
				}

				for (int i2 = 0; i2 < i; i2++) {  //ѕроизводим деление термов

					if (node[i][j].Get_index()[i2 + 1] - '0' == 0)   //≈сли левый узел
					{
					...
					else
					{
						...
						else
						{
							...
							{
								//ѕриравниваем начало к концу => 0 новых правил
				...
				int n_rules = 1;
				for (int v = 0; v < n_var; v++) { //—читаем кол-во правил
